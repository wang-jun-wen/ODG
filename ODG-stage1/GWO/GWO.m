...
function [Alpha_score,Alpha_pos,Convergence_curve]=GWO(SearchAgents_no,Max_iteration,lb,ub,dim,data_group)
%%  优化算法初始化
...
while iter < Max_iteration           % 对迭代次数循环
    for i = 1 : size(Positions, 1)   % 遍历每个狼
        ...
        % 计算适应度函数值
        fitness = getfitness_average(Positions(i, :),data_group);  %对十组数据同时进行训练，在验证集上验证.求平均的MAE作为适应度值返回
        % 更新 Alpha, Beta, Delta
